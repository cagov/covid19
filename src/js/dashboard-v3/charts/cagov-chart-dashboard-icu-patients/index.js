...
class CAGovDashboardICUPatients extends window.HTMLElement {
  connectedCallback() {
    console.log("Loading CAGovDashboardICUPatients");
    this.translationsObj = getTranslations(this);

    // Settings and initial values
    this.chartOptions = {
      chartName: 'cagov-chart-dashboard-icu-patients',
      // Data
      dataUrl:
        config.chartsStateDashTablesLoc + "icu-patients/california.json", // Overwritten by county.
      dataUrlCounty:
        config.chartsStateDashTablesLoc + "icu-patients/<county>.json",
  ...
  getTooltipContent(di) {
    const barSeries = this.chartdata.time_series.ICU_PATIENTS;
    const lineSeries = this.chartdata.time_series.ICU_PATIENTS_14_DAY_AVG;
    ...
    window
      ...
      .then(
        function (alldata) {
          ...
          this.translationsObj.post_chartLegend2 = applySubstitutions(this.chartdata.latest.ICU_PATIENTS.CHANGE_FACTOR >= 0? this.translationsObj.chartLegend2Increase : this.translationsObj.chartLegend2Decrease, repDict);
        ...
        renderChart.call(this, {'tooltip_func':this.tooltip,
                                ...
                                'time_series_bars':this.chartdata.time_series['ICU_PATIENTS'],
                                'time_series_line':this.chartdata.time_series['ICU_PATIENTS_14_DAY_AVG'],
                                'root_id':'hosp-p',
