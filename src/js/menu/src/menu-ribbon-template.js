    ...
    <nav role="navigation" class="${classesNav}" aria-label="Site Navigation">
      ...
      ${data.sections.map(section => `
        <div class="${classesPrimary}">
          <button class="menu-ribbon--button" aria-controls="primary-links-${section.idx}">${section.title}
          ...
          L0.7,5.7C0.2,5.2,0,4.6,0,3.9S0.2,2.7,0.7,2.2z"></path></svg>
            ...
            <!-- Secondary -->
            <ul class="${classesSecondary}" role="menu" id="secondary-links-${section.idx}">
              ${section.links.map(link => `<li><a href="${link.url}">${link.name}</a></li>`).join(' ')}
            </ul>
        </div>
      `)
    .join(' ')}
