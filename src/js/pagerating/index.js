...
class CWDSPageRating extends window.HTMLElement {
  connectedCallback () {
    let question = (this.dataset.question ? this.dataset.question : 'Did you find what you were looking for?');
    let yes = (this.dataset.yes ? this.dataset.yes : 'Yes');
    let no = (this.dataset.no ? this.dataset.no : 'No');
    let commentPrompt = (this.dataset.commentPrompt ? this.dataset.commentPrompt : 'What was the problem?');
