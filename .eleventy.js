...
const langData = JSON.parse(
  fs.readFileSync("pages/_data/langData.json", "utf8")
);
const statsData = JSON.parse(
  fs.readFileSync("pages/_data/caseStats.json", "utf8")
).Table1[0];
let htmlmap = [];
...
const langPostfixRegExp = new RegExp(
  `(?:${langData.languages
    .map((x) => x.filepostfix)
    .filter((x) => x)
    .join("|")})$`
...
module.exports = function (eleventyConfig) {
    ...
    collection.getAll().forEach((item) => {
      if (item.inputPath.includes(manualContentFolderName)) {
        item.outputPath = item.outputPath.replace(
          `/${manualContentFolderName}`,
          ""
        );
        item.url = item.url.replace(`/${manualContentFolderName}`, "");
        item.data.page.fileSlug = item.data.page.fileSlug.replace(
          manualContentFolderName,
          ""
        );
        item.data.page.url = item.data.page.url.replace(
          `/${manualContentFolderName}`,
          ""
        );
        output.push(item);
  ...
  eleventyConfig.addFilter("find", (array, field, value) =>
    array.find((x) => x[field] === value)
  );
  ...
  eleventyConfig.addFilter("formatDate", function (datestring) {
    ...
    if (datestring && datestring.indexOf("Z") > -1) {
      ...
      return `${date.toLocaleDateString(locales, {
        ...
        month: "long",
        year: "numeric",
      })} at ${date.toLocaleTimeString(locales, {
        timeZone,
        hour: "numeric",
        minute: "numeric",
      })}`;
  ...
  eleventyConfig.addFilter(
    "_statsdata_",
    (index) => Object.values(statsData)[index]
  );
  //Usage...
  //        {{0|_statsdata_}}
  ...
  const contentfrompage = (content, page, slug) => {
    if (
      page.fileSlug &&
      slug &&
      page.fileSlug.toLocaleLowerCase().startsWith(slug.toLocaleLowerCase())
    ) {
      return content;
    }
    return "";
  ...
  eleventyConfig.addFilter("pageActive", (page, tags, pageObj) =>
    contentfrompage(" active", page, getTranslatedValue(pageObj, tags, "slug"))
  );
    ...
    if (
      ...
      if (processedPostMap.get(outputPath) !== initialHTML) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify([...processedPostMap]),
          "utf8"
        );
  ...
  eleventyConfig.addTransform("findlinkstolocalize", async function (
    html,
    outputPath
  ) {
    ...
    if (
      outputPath &&
      outputPath.endsWith(".html") &&
      html.indexOf(localizeString) > -1
    ) {
      const dom = new JSDOM(html);
      const document = dom.window.document;
      let lang = langData.languages
        .filter(
          (x) => x.enabled && document.querySelector("html").lang == x.hreflang
        )
        .concat(langData.languages[0])[0].id;
      if (lang !== "en") {
        for (const image of document.querySelectorAll(
          `img[src*='${localizeString}']`
        )) {
          let englishUrl = image.src;
          let localizedUrl = englishUrl.replace(
            "--en.",
            `--${lang.toLowerCase()}.`
          );
          let localizedUri = localizedUrl.replace(
            "https://files.covid19.ca.gov",
            ""
          );
  ...
  eleventyConfig.addFilter(
    ...
    (page) =>
      (page.data ? page.data.publishdate : page.publishdate) ||
      page.date.toISOString()
  );

  eleventyConfig.addPairedShortcode(
    "dothisifcontentexists",
    (content, contentcontent, match) =>
  ...
  eleventyConfig.addFilter("getAltPageRows", (page) => {
    ...
    if (
      langData.languages.some((x) => engSlug === x.filepostfix.substring(1))
    ) {
